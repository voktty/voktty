...
Voktty runs shells, reads and writes files, and sends data to AI providers. The security model is defense-in-depth: no single guard is enough, so every boundary validates input before acting on it.
...
## External MCP tool boundary

MCP servers are untrusted code and untrusted data sources. `src-tauri/src/modules/mcp/` owns their processes, network access, credentials, schemas and execution policy. The webview cannot turn an approval card into authority: Rust issues an opaque, expiring, single-use grant bound to the exact snapshot, namespaced tool, effects, scope and input digest, then consumes it before dispatch.

Remote names, descriptions, annotations, schemas, prompts and results cannot grant trust. An annotation may only add effects. Automatic execution is limited to an exact tool name that the user explicitly marked as a read and whose effective effect set remains exactly `read`. Every other call requires approval. Input is validated before IPC and again natively; structured output is validated before it reaches the model.

Descriptions and results are labeled as untrusted model data. This does not claim that prompt injection can be detected reliably. Instead, native policy, exact scopes, one-use grants and bounded transports ensure that hostile model text cannot itself widen authority or bypass approval.

stdio starts executables without a shell, inside an authorized root and with a minimal environment. Streamable HTTP validates every destination and redirect, pins DNS, blocks metadata targets and binds credentials to the original audience. Disabling, deleting, restarting or revoking a server invalidates snapshots and cancels active work. Audit records exclude arguments, output, authorization challenges and secrets.
...
Local LLM endpoints are explicitly allowed because the user opted in by pointing Voktty at them, but they are still classified and logged.
...
The agent detector (`src-tauri/src/modules/pty/agent_detect.rs`) is armed by `OSC 133;C;<cmd>` or by a self-armed marker and emits `voktty:agent-signal` events. It is driven **only by OSC sequences**, never by raw output, so a repainting TUI never flaps.
