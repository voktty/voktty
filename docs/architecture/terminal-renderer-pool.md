...
Terminal tabs are kept mounted and hidden on switch so PTYs and dev servers keep streaming in the background. Creating an unbounded number of live xterm + WebGL renderer instances would blow the memory budget, so Voktty pools renderer slots.
...
If only a snapshot exists, `bindSlot` hides the slot, clears the terminal, resizes, writes the snapshot, then drains the ring. The slot becomes visible only after xterm confirms that all queued replay writes were parsed. For alt-screen TUIs, the snapshot is skipped and a SIGWINCH kick is sent so the TUI repaints from scratch.
