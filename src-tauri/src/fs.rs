...
#[tauri::command]
pub async fn git_session_checkout(
    cwd: String,
    name: String,
    remote: Option<String>,
) -> Result<GitSessionCheckout, String> {
    tauri::async_runtime::spawn_blocking(move || {
        git_session_checkout_for(&expand_home(&cwd), &name, remote.as_deref())
    })
    .await
    .map_err(|e| e.to_string())?
}

/// Create a branch from HEAD in a session worktree without moving the project HEAD.
#[tauri::command]
pub async fn git_session_create_branch(
    cwd: String,
    name: String,
) -> Result<GitSessionCheckout, String> {
    tauri::async_runtime::spawn_blocking(move || {
        git_session_create_branch_for(&expand_home(&cwd), &name)
    })
    .await
    .map_err(|e| e.to_string())?
}
...
fn git_session_checkout_for(
    ...
    let name = git_ensure_local_branch(root, name, remote)?;
    if let Some(existing) = git_worktree_for_branch(root, &name) {
        return Ok(git_session_checkout_result(root, &name, &existing));
    }
    let path = git_new_worktree_path(root, &name)?;
    git_checked(root, &["worktree", "add", &path.to_string_lossy(), &name])?;
    Ok(git_session_checkout_result(root, &name, &path))
}

fn git_session_create_branch_for(root: &Path, name: &str) -> Result<GitSessionCheckout, String> {
    if !git_is_work_tree(root) {
        return Err("Not a git repository".into());
    }
    let name = git_branch_name(root, name)?;
...
fn git_session_checkout_result(root: &Path, branch: &str, cwd: &Path) -> GitSessionCheckout {
    let cwd = cwd
        .canonicalize()
...
fn git_new_worktree_path(root: &Path, branch: &str) -> Result<PathBuf, String> {
    let common = git_common_dir(root).ok_or_else(|| "Not a git repository".to_string())?;
    let base = common.join("monocode-worktrees").join(branch);
    if !base.exists() {
    ...
    for index in 2..32 {
        let candidate = common
            .join("monocode-worktrees")
            .join(format!("{branch}-{index}"));
        if !candidate.exists() {
    ...
    Err(format!("Could not allocate a worktree for {branch}"))
}
...
fn git_worktrees(root: &Path) -> Vec<GitWorktree> {
    ...
    for line in text.lines() {
        if line.is_empty() {
            push_worktree(&mut out, &mut path, &mut branch);
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn git_session_checkout_keeps_project_head() {
        let dir = tmp("git-session-branch");
        if !init_git_commit(&dir.0, &[("a.txt", "alpha\n")]) {
            return;
        }
        let created = git_session_create_branch_for(&dir.0, "feat/picker").unwrap();
        assert_eq!(created.branch, "feat/picker");
        assert!(created.isolated);
        assert_ne!(
            PathBuf::from(&created.cwd).canonicalize().ok(),
            dir.0.canonicalize().ok()
        );
        assert_eq!(git_head_branch(&dir.0).as_deref(), Some("main"));
        assert_eq!(
            git_head_branch(Path::new(&created.cwd)).as_deref(),
            Some("feat/picker")
        );

        let reused = git_session_checkout_for(&dir.0, "feat/picker", None).unwrap();
        assert_eq!(reused.branch, "feat/picker");
        assert!(reused.isolated);
        assert_eq!(
            PathBuf::from(&reused.cwd).canonicalize().ok(),
            PathBuf::from(&created.cwd).canonicalize().ok()
        );
        assert_eq!(git_head_branch(&dir.0).as_deref(), Some("main"));

        let main = git_session_checkout_for(&dir.0, "main", None).unwrap();
        assert_eq!(main.branch, "main");
        assert!(!main.isolated);
        assert_eq!(
            PathBuf::from(&main.cwd).canonicalize().ok(),
            dir.0.canonicalize().ok()
        );
        assert_eq!(git_head_branch(&dir.0).as_deref(), Some("main"));
        assert!(git_session_create_branch_for(&dir.0, "feat/picker").is_err());
    }

    #[test]
    fn git_session_checkout_tracks_remote_without_moving_head() {
        let repo = tmp("git-session-remote-repo");
        let origin = tmp("git-session-remote-origin");
        if !init_git_commit(&repo.0, &[("a.txt", "alpha\n")]) {
            return;
        }
        if Command::new("git")
        ...
        let checked = git_session_checkout_for(&repo.0, "feature", Some("origin")).unwrap();
        assert_eq!(checked.branch, "feature");
        assert!(checked.isolated);
        assert_eq!(git_head_branch(&repo.0).as_deref(), Some("main"));
        assert_eq!(
            git_head_branch(Path::new(&checked.cwd)).as_deref(),
            Some("feature")
        );
    }
}
