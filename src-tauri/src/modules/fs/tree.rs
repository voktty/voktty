...
use super::{blocking_fs, is_network_path};
...
pub async fn list_subdirs(
    ...
    let root = resolve_path(&path, &workspace);
    let network_path = is_network_path(&root);
    blocking_fs(network_path, move || list_subdirs_sync(root, show_hidden)).await
}

fn list_subdirs_sync(root: std::path::PathBuf, show_hidden: bool) -> Result<Vec<String>, String> {
    let read = std::fs::read_dir(&root).map_err(|e| {
