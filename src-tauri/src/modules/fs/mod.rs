...
const NETWORK_FS_CONCURRENCY: usize = 2;
const NETWORK_FS_QUEUE_TIMEOUT: Duration = Duration::from_secs(5);
static NETWORK_FS_LIMIT: OnceLock<tokio::sync::Semaphore> = OnceLock::new();

pub(super) async fn blocking_fs<F, T>(network_path: bool, operation: F) -> Result<T, String>
where
    F: FnOnce() -> Result<T, String> + Send + 'static,
    T: Send + 'static,
{
    let _network_permit = if network_path {
        let semaphore =
            NETWORK_FS_LIMIT.get_or_init(|| tokio::sync::Semaphore::new(NETWORK_FS_CONCURRENCY));
        Some(
            tokio::time::timeout(NETWORK_FS_QUEUE_TIMEOUT, semaphore.acquire())
                .await
                .map_err(|_| "network filesystem is busy or unavailable".to_string())?
                .map_err(|e| format!("network filesystem limiter closed: {e}"))?,
        )
    } else {
        None
    };
    tauri::async_runtime::spawn_blocking(operation)
        .await
        .map_err(|e| format!("filesystem worker failed: {e}"))?
}
...
#[cfg(not(windows))]
pub(super) fn is_network_path(_path: &Path) -> bool {
    false
}
