...
fn status_arguments() -> [&'static str; 5] {
    [
        "status",
        "--porcelain=v2",
        "--branch",
...
fn safe_directory_path(path: &str) -> Result<&str> {
    let clean_path = path.trim().trim_matches('\'').trim_matches('"');
    if clean_path.is_empty() || clean_path.chars().any(char::is_control) {
        return Err(GitError::InvalidPath("invalid safe directory path".into()));
    }
    Ok(clean_path)
}
...
mod tests {
    ...
    #[test]
    fn status_uses_bounded_untracked_directory_enumeration() {
        assert_eq!(status_arguments().last(), Some(&"--untracked-files=normal"));
    }

    #[test]
    fn safe_directory_path_rejects_empty_and_control_characters() {
        assert!(safe_directory_path("  ").is_err());
        assert!(safe_directory_path("/repo\nother").is_err());
        assert_eq!(safe_directory_path("'/srv/repo'").unwrap(), "/srv/repo");
    }
