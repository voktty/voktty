    ...
    S: AsRef<OsStr>,
{
    ...
    if output.exit_code != Some(0) {
        ...
        if let Some(err) = classify_git_error(&stderr) {
            return Err(err);
        }
        return Ok(None);
    }
...
pub fn classify_git_error(stderr: &str) -> Option<GitError> {
...
mod tests {
    ...
    fn builds_wsl_git_command_with_cd_and_exec() {
        let cmd = build_git_command(
            ...
            Some("/home/devuser/Nova pasta/repo"),
