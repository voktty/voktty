...
enum Delivery {
    ...
    Osc,
    // Kimi Code: writes to ~/.kimi-code/config.toml in [[hooks]] array.
    Toml,
}
...
const PI_EXTENSION_FILE: &str = "voktty-notifications.ts";
const PI_EXTENSION_MARKER: &str = "voktty-pi-notifications-v1";
const PI_STATUS_NEEDLES: [&str; 6] = [
    PI_EXTENSION_MARKER,
    "agent_start",
    "agent_settled",
    "notify;Voktty;pi;${event}",
    "emit(\"working\")",
    "emit(\"finished\")",
];
const PI_EXTENSION: &str = r#"// voktty-pi-notifications-v1
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

export default function (pi: ExtensionAPI) {
  const emit = (event: "working" | "finished") => {
    if (process.env.VOKTTY_TERMINAL) {
      process.stdout.write(`\u001b]777;notify;Voktty;pi;${event}\u0007`);
    }
  };
...
fn hook_command(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!(
            r#"[ -n "$VOKTTY_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;Voktty;{event}\\u0007"}}' || true"#
        ),
...
fn osc_command(agent: &str, event: &str) -> String {
    format!(
        r#"[ -n "$VOKTTY_TERMINAL" ] && printf '\033]777;notify;Voktty;{agent};{event}\007' > /dev/tty; printf '{{}}'"#
    )
}
...
fn osc_command(agent: &str, event: &str) -> String {
    let exe = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "voktty.exe".to_string());
    format!(r#""{exe}" __voktty_notify {agent} {event}"#)
}

#[cfg(unix)]
fn kimi_osc_command(agent: &str, event: &str) -> String {
    format!(
        r#"[ -n "$VOKTTY_TERMINAL" ] && printf '\033]777;notify;Voktty;{agent};{event}\007' > /dev/tty; cat >/dev/null || true"#
    )
}

#[cfg(windows)]
fn kimi_osc_command(agent: &str, event: &str) -> String {
    osc_command(agent, event)
}
...
fn status_needle(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!("notify;Voktty;{event}"),
        Delivery::Osc | Delivery::Toml => {
            #[cfg(unix)]
            {
                format!("notify;Voktty;{};{event}", spec.agent)
            }
            #[cfg(windows)]
            {
                format!("__voktty_notify {} {event}", spec.agent)
            }
        }
    }
}
    ...
    while let Some(line) = lines.next() {
        if line.trim() == "[[hooks]]" {
            ...
            let owned = block.iter().any(|l| {
                let t = l.trim_start();
                t.starts_with("command") && t.contains('=') && is_owned_hook_command(t)
            });
            if owned {
                continue;
            }
...
fn pi_extension_contents(
    ...
    if existing.is_some_and(|s| !s.trim().is_empty() && !s.contains(PI_EXTENSION_MARKER)) {
        return Err(format!(
            "{} is not managed by Voktty; refusing to overwrite",
            path.display()
        ));
    }
    Ok(PI_EXTENSION)
...
fn write_atomic(path: &std::path::Path, contents: &str) -> Result<(), String> {
    let tmp = path.with_extension("voktty-tmp");
...
fn conout_marker(agent: &str, event: &str) -> String {
    format!("\x1b]777;notify;Voktty;{agent};{event}\x07")
}

// Windows has no /dev/tty: the hook calls `voktty.exe __voktty_notify ...` and we
...
pub fn emit_conout_marker(agent: &str, event: &str) {
    ...
    if std::env::var_os("VOKTTY_TERMINAL").is_none() {
        return;
    }
    unsafe {
...
mod tests {
    ...
    fn claude_adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;Voktty;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;Voktty;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;Voktty;working"));
    ...
    fn is_idempotent_per_agent() {
        ...
        let s = spec("kimi");
        let once = merge_kimi_hooks("", s);
        let twice = merge_kimi_hooks(&once, s);
        assert_eq!(once, twice, "kimi not idempotent");
    }

    #[test]
    fn kimi_merges_hooks_properly() {
        let s = spec("kimi");
        let out = merge_kimi_hooks("[theme]\nname = \"dark\"\n", s);
    ...
    fn conout_marker_matches_detector_format() {
        ...
        assert_eq!(
            ...
            "\u{1b}]777;notify;Voktty;gemini;attention\u{7}"
        );
    }
    ...
    fn codex_emits_four_field_dev_tty_marker() {
        ...
        assert!(stop.contains("notify;Voktty;codex;finished"));
    ...
    fn gemini_uses_matcher_and_named_marker() {
        ...
        assert!(command(&out, "AfterAgent", 0).contains("notify;Voktty;gemini;finished"));
        assert!(command(&out, "Notification", 0).contains("notify;Voktty;gemini;attention"));
    }

    #[test]
    fn pi_extension_emits_named_working_and_finished_markers() {
        let path = std::path::Path::new("/x/voktty-notifications.ts");
        ...
        assert!(extension.contains("process.env.VOKTTY_TERMINAL"));
        assert!(extension.contains("process.stdout.write"));
    }

    #[test]
    fn pi_extension_only_replaces_voktty_owned_file() {
        let path = std::path::Path::new("/x/voktty-notifications.ts");
    ...
    fn pi_extension_install_is_atomic_idempotent_and_preserves_foreign_files() {
        let dir = std::env::temp_dir().join(format!("voktty-pi-extension-{}", std::process::id()));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$VOKTTY_TERMINAL\" ] && printf '\\033]777;voktty;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
