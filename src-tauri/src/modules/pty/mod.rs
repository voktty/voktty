...
impl PtyState {
    ...
    pub(crate) fn has_session(&self, id: u32) -> bool {
        self.sessions.read().unwrap().contains_key(&id)
    }

    pub(crate) fn write_bytes(&self, id: u32, bytes: &[u8]) -> Result<(), String> {
        let session = self
            .sessions
            .read()
            .unwrap()
    ...
    pub(crate) fn send_collab_snapshot_barrier(
        &self,
        id: u32,
        token: &str,
        sequence: impl FnOnce() -> Result<u64, String>,
    ) -> Result<u64, String> {
        if token.is_empty()
            || token.len() > 64
            || !token
        ...
        {
            return Err("invalid snapshot barrier token".to_string());
        }
        let session = self
            .sessions
            .read()
        ...
        let _output_guard = session.output_gate.lock().unwrap();
        let sequence = sequence()?;
        let marker = format!("\0VOKTTY_COLLAB_SNAPSHOT:{token}").into_bytes();
        session
            .on_data
            .send(Response::new(marker))
            .map_err(|_| "terminal snapshot barrier channel is closed".to_string())?;
        Ok(sequence)
    }
}
...
pub async fn pty_open(
    ...
    let control_env = if workspace.is_wsl() || workspace.is_remote() {
        None
    } else {
...
pub fn pty_resize(
    state: tauri::State<PtyState>,
    collab: tauri::State<crate::modules::collab::CollabState>,
    id: u32,
    cols: u16,
    rows: u16,
) -> Result<(), String> {
    ...
    collab.resize_terminal(id, cols, rows);
    Ok(())
}
...
pub fn pty_close(
    ...
    collab.stop_host(id, "terminal_closed");
...
pub fn pty_close_all(
    ...
    collab.stop_all("terminals_closed");
