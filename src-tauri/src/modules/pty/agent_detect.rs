...
// (Claude) or 4-field `notify;Voktty;<agent>;<event>` (Codex/Gemini/Pi).
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(tail) = pt
            .strip_prefix(VOKTTY_AGENT_MARKER)
            .or_else(|| pt.strip_prefix(LEGACY_AGENT_MARKER))
        {
    ...
    fn match_agent(&self, cmd: &[u8]) -> Option<String> {
        ...
        for token in cmd.split_whitespace() {
            ...
            let unextended = lower_base
                .strip_suffix(".cmd")
                .or_else(|| lower_base.strip_suffix(".exe"))
                .or_else(|| lower_base.strip_suffix(".ps1"))
                .or_else(|| lower_base.strip_suffix(".bat"))
                .or_else(|| lower_base.strip_suffix(".js"))
                .or_else(|| lower_base.strip_suffix(".mjs"))
                .or_else(|| lower_base.strip_suffix(".cjs"))
                .or_else(|| lower_base.strip_suffix(".sh"))
                .unwrap_or(&lower_base);

            let clean = unextended.trim_start_matches('@');
            let clean_base = clean.rsplit('/').next().unwrap_or(clean);

            if let Some(agent) = self.agents.iter().find(|a| {
                let a_low = a.to_lowercase();
                clean_base == a_low
                    || clean_base.starts_with(&format!("{a_low}-"))
                    || clean_base.starts_with(&format!("{a_low}_"))
...
mod tests {
    ...
    fn arms_on_pathed_and_wrapped_command() {
        ...
        assert_eq!(
            run(&mut d3, &osc("133;C;codex.cmd")),
            vec![started("codex")]
        );
        let mut d4 = AgentDetector::new();
        assert_eq!(
            run(&mut d4, &osc("133;C;codex.ps1")),
            vec![started("codex")]
        );
        let mut d5 = AgentDetector::new();
        assert_eq!(
            run(&mut d5, &osc("133;C;npx @openai/codex")),
            vec![started("codex")]
        );
    }
    ...
    #[test]
    fn voktty_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;attention")),
            vec![Transition::Attention]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;working")),
            vec![Transition::Working]
        );
        assert!(run(&mut d, &osc("777;notify;Voktty;working")).is_empty());
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;finished")),
            vec![Transition::Finished]
        );
    }

    #[test]
    fn voktty_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;attention")),
    ...
    fn four_field_marker_self_arms_named_agent() {
        ...
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;codex;working")),
            vec![started("codex")]
        );
        let mut g = AgentDetector::new();
        assert_eq!(
            run(&mut g, &osc("777;notify;Voktty;gemini;finished")),
    ...
    fn pi_marker_self_arms_and_drives_status() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;pi;working")),
            vec![started("pi")]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;pi;finished")),
            vec![Transition::Finished]
        );
    }
    ...
    fn four_field_marker_ignores_unknown_agent() {
        let mut d = AgentDetector::new();
        assert!(run(&mut d, &osc("777;notify;Voktty;evil;attention")).is_empty());
        // A known agent in the same chunk still works.
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;codex;attention")),
    ...
    fn four_field_marker_drives_status_after_preexec() {
        ...
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;gemini;attention")),
            vec![Transition::Attention]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;gemini;working")),
            vec![Transition::Working]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;Voktty;gemini;finished")),
            vec![Transition::Finished]
        );
    }
