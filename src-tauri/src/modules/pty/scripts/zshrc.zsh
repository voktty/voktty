# voktty-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_voktty_ret`.

{
  _voktty_user_zdotdir="${VOKTTY_USER_ZDOTDIR:-$HOME}"
  [ -f "$_voktty_user_zdotdir/.zshrc" ] && source "$_voktty_user_zdotdir/.zshrc"
  unset _voktty_user_zdotdir
}

if [[ -n "$VOKTTY_CLI" && -x "$VOKTTY_CLI" ]]; then
  voktty() {
    command "$VOKTTY_CLI" "$@"
  }
fi
...
if [[ -z "$__VOKTTY_HOOKS_LOADED" ]]; then
  __VOKTTY_HOOKS_LOADED=1
  ...
  _voktty_preexec() {
    ...
    [[ -n "$VOKTTY_BLOCKS" ]] && _voktty_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _voktty_precmd
    add-zsh-hook preexec _voktty_preexec
  fi
