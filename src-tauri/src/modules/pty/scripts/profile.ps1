# voktty-shell-integration (PowerShell)
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__voktty_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__voktty_urlencode {
    param([string]$s)
...
function global:__voktty_install_readline {
    if ($global:__voktty_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__voktty_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__voktty_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __voktty_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__voktty_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__voktty_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    $original = if (Test-Path Function:__voktty_user_prompt) {
        try { & __voktty_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
