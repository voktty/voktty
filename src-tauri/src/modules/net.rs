...
pub(crate) struct SafeHttpTarget {
    pub url: reqwest::Url,
...
pub(crate) async fn resolve_safe_http_target(
    url: &str,
    allow_private: bool,
) -> Result<SafeHttpTarget, String> {
    let parsed = validate_url(url, allow_private)?;
    let host = parsed
        .host_str()
        .ok_or_else(|| "missing host".to_string())?
        .to_string();
    let (kind, resolved) = resolve_and_classify(&host).await?;
    match kind {
        IpKind::BlockedMetadata => return Err(format!("host not allowed: {host}")),
        IpKind::Loopback | IpKind::Private if !allow_private => {
            return Err(format!(
    ...
    Ok(SafeHttpTarget {
        url: parsed,
        is_private_network: matches!(kind, IpKind::Loopback | IpKind::Private),
        host,
        ips,
    })
}
