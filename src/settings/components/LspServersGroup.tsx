  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <Label>{t("settings.editor.lsp.languageServersLabel")}</Label>
...
function ServerRow({
  ...
  const status =
    detected === undefined
      ? t("settings.editor.lsp.checkingStatus")
      : detected
        ? detected
        : t("settings.editor.lsp.notFoundStatus");
      ...
      <div className="flex items-center gap-1.5">
        ...
        {custom ? (
          <button
            ...
            title={t("settings.editor.lsp.removeServer")}
          >
        ...
        <Switch
          ...
          aria-label={t("settings.editor.lsp.toggleServerAria", { action: switchState.checked ? t("lsp.disable") : t("common.enable"), name: server.name })}
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="h-6 px-2 text-[11px]">
          {t("settings.editor.lsp.addServer")}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-sm">{t("settings.editor.lsp.customServerDialogTitle")}</DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-2.5">
          {field(t("settings.editor.lsp.serverName"), name, setName, "Zig")}
          {field(t("settings.editor.lsp.serverCommand"), command, setCommand, "zls")}
          {field(t("settings.editor.lsp.serverArgs"), args, setArgs, "--stdio")}
          {field(t("settings.editor.lsp.serverLanguages"), extensions, setExtensions, "zig, zon")}
          {field(t("settings.editor.lsp.languageIdLabel"), languageId, setLanguageId, "zig")}
          {field(t("settings.editor.lsp.serverRootMarkers"), rootMarkers, setRootMarkers, "build.zig")}
        </div>
        <DialogFooter>
          <Button size="sm" disabled={!valid} onClick={save}>
            {t("settings.editor.lsp.addServerSubmit")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
