  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{t("settings.editor.lsp.installDialogTitle", { name: server.name })}</DialogTitle>
          <DialogDescription>
            {t("settings.editor.lsp.dialogNotFound", { command: server.command })}
          </DialogDescription>
        </DialogHeader>
        ...
        {server.install ? (
          <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2 font-mono text-[11px]">
            ...
            <button
              ...
              title={t("settings.editor.lsp.copyInstallCommand")}
            >
              <HugeiconsIcon
          ...
          <p className="rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            {t("settings.editor.lsp.manualInstallHint")}
          </p>
        )}

        {notFound ? (
          <p className="text-xs text-destructive">
            {t("settings.editor.lsp.stillNotFound")}
          </p>
        ) : null}
