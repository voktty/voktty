...
const MAX_HIGHLIGHT_CACHE_CODE_CHARS = 64_000;

function highlightCacheKey(code: string, lang: string): string {
  let hash = 2_166_136_261;
  for (let index = 0; index < code.length; index += 1) {
    hash ^= code.charCodeAt(index);
    hash = Math.imul(hash, 16_777_619);
  }
  return `${lang}:${code.length}:${hash >>> 0}`;
}

function cachedHighlight(code: string, lang: string): HighlightedNode[] | null {
  if (code.length > MAX_HIGHLIGHT_CACHE_CODE_CHARS) return null;
  const key = highlightCacheKey(code, lang);
  const cached = highlightCache.get(key);
  if (!cached || cached.code !== code || cached.lang !== lang) return null;
  highlightCache.delete(key);
  highlightCache.set(key, cached);
  return cached.nodes;
}

function cacheHighlight(
  code: string,
  lang: string,
  nodes: HighlightedNode[],
): void {
  if (code.length > MAX_HIGHLIGHT_CACHE_CODE_CHARS) return;
  const key = highlightCacheKey(code, lang);
  highlightCache.delete(key);
  highlightCache.set(key, { code, lang, nodes });
  ...
  return (
    <div className="not-prose my-2 flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-2 text-[11px] text-muted-foreground">
      ...
      <Shimmer duration={1.2}>
        {label === "text"
          ? t("ai.generatingCode")
          : t("ai.generatingLanguage", { language: label })}
      </Shimmer>
    </div>
  );
}
...
const HighlightedPre = memo(function HighlightedPre({
  ...
  const [nodes, setNodes] = useState<HighlightedNode[] | null>(() =>
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const cached = cachedHighlight(code, lang);
    if (cached) {
      setNodes(cached);
      return () => {
    ...
    highlight(code, lang)
      .then((result) => {
        ...
        if (result) cacheHighlight(code, lang, result);
        setNodes(result);
      })
      .catch(() => {
        if (cancelled) return;
...
function RunInTerminalButton({ command }: { command: string }) {
  ...
  return (
    <Button
      ...
      aria-label={t("tooltips.runInActiveTerminal")}
      title={t("tooltips.runInActiveTerminal")}
    >
      <HugeiconsIcon
        ...
        strokeWidth={1.75}
      />
      <span>{sent ? t("tooltips.sent") : t("terminal.scripts.run")}</span>
    </Button>
  );
}
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <Button
      ...
      aria-label={t("tooltips.copyCode")}
    >
      <HugeiconsIcon
