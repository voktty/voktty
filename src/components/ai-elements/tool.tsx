...
const TOOL_META: Record<string, { labelKey: string; icon: typeof File01Icon }> = {
  read_file: { labelKey: "read", icon: File01Icon },
  list_directory: { labelKey: "list", icon: FolderOpenIcon },
  write_file: { labelKey: "write", icon: FilePlusIcon },
  create_directory: { labelKey: "createDirectory", icon: FolderAddIcon },
  edit: { labelKey: "edit", icon: FileEditIcon },
  multi_edit: { labelKey: "edit", icon: Edit02Icon },
  bash_run: { labelKey: "run", icon: TerminalIcon },
  bash_background: { labelKey: "spawn", icon: TerminalIcon },
  bash_logs: { labelKey: "logs", icon: TerminalIcon },
  bash_list: { labelKey: "jobs", icon: TerminalIcon },
  bash_kill: { labelKey: "kill", icon: TerminalIcon },
  grep: { labelKey: "search", icon: GlobalSearchIcon },
  glob: { labelKey: "glob", icon: Folder01Icon },
  suggest_command: { labelKey: "suggest", icon: SparklesIcon },
  open_preview: { labelKey: "preview", icon: EyeIcon },
  run_subagent: { labelKey: "subagent", icon: RobotIcon },
  todo_write: { labelKey: "todos", icon: CheckListIcon },
};
...
const ToolImpl = ({
  ...
  const label = meta
    ? t(`ai.tools.names.${meta.labelKey}`)
    : toolName;
  ...
  const statusLabel =
    state === "approval-requested"
      ? t("ai.tools.status.awaitingApproval")
      : state === "approval-responded"
        ? t("ai.tools.status.responded")
        : state === "input-streaming"
          ? t("ai.tools.status.preparing")
          : state === "input-available"
            ? t("ai.tools.status.running")
            : state === "output-available"
              ? t("ai.tools.status.done")
              : state === "output-denied"
                ? t("ai.tools.status.denied")
                : state === "output-error"
                  ? t("ai.tools.status.error")
                  : state;
  ...
  return (
    ...
    >
      ...
      {hasDetails && (
        <CollapsibleContent
          className={cn("voktty-collapsible-content")}
        >
...
function renderToolOutput(
  ...
  if (toolName === "read_file") {
    ...
    return (
      <div className="flex items-center gap-1.5 font-mono text-[11px]">
        ...
        <span className="text-foreground">{t("ai.tools.labels.read")}</span>
  ...
  if (toolName === "grep") {
    ...
    if (hits.length === 0) {
      return (
        <div className="text-[11px] italic text-muted-foreground">
          {t("ai.tools.labels.noMatches")}
          {filesScanned != null
            ? ` ${t("ai.tools.labels.filesScanned", { count: filesScanned })}`
            : ""}
        </div>
      );
    }
    ...
    return (
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          <span>
            {t("ai.tools.labels.hitsCount", { count: hits.length })}
            {filesScanned != null
              ? ` ${t("ai.tools.labels.filesCount", { count: filesScanned })}`
              : ""}
          </span>
          {truncated ? (
            <span className="rounded bg-amber-500/15 px-1.5 py-0.5 text-amber-700 dark:text-amber-400">
              {t("ai.tools.labels.truncated")}
            </span>
          ) : null}
        </div>
      </div>
        ...
        <div className="flex items-center gap-1.5 font-mono text-[11px]">
          ...
          {reps != null ? (
            <span className="text-foreground">
              {t("ai.tools.labels.replacementCount", { count: reps })}
            </span>
          ) : null}
          {path ? (
  ...
  if (toolName === "write_file" || toolName === "create_directory") {
    ...
    return (
      <div className="flex items-center gap-1.5 font-mono text-[11px]">
        ...
        <span className="text-foreground">
          {toolName === "create_directory"
            ? t("ai.tools.labels.created")
            : t("ai.tools.labels.wrote")}
        </span>
        {path ? <span className="text-muted-foreground">· {path}</span> : null}
        {bytes != null ? (
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-muted-foreground">{t("ai.tools.labels.running")}</span>
        </div>
        {cmd ? (
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-stretch gap-1.5 rounded bg-muted/40 overflow-hidden">
        ...
        <button
          ...
          aria-label={t("ai.tools.labels.insertAria")}
        >
          <HugeiconsIcon
          ...
          <span>
            {inserted
              ? t("ai.tools.labels.inserted")
              : t("ai.tools.labels.insert")}
          </span>
        </button>
      </div>
    </div>
