  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[auto_auto] items-center gap-x-12 gap-y-3 text-[13px]">
        <Hint label={t("terminal.watermark.historyHint")} keys="↑" />
        <Hint label={t("terminal.watermark.autocompleteHint")} keys="Tab" />
        <Hint
          label={t("terminal.watermark.switchHint")}
          shortcut="terminal.toggleInput"
        />
        {aiAvailable ? (
          <Hint
            label={t("terminal.watermark.openAiHint")}
            shortcut="ai.toggle"
          />
        ) : null}
      </div>
