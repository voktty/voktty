...
import { PROVIDERS, type ProviderId } from "@/modules/ai/config";
import { getKey, setKey } from "@/modules/ai/lib/keyring";
import { useTranslation } from "@/modules/i18n";
import { type LanguageId, SUPPORTED_LANGUAGES } from "@/modules/i18n/types";
import {
  setDefaultModel,
  setEditorFontSize,
...
// The onboarding grid only covers providers that take a pasted API key.
// openai-compatible/lmstudio/mlx/ollama need a base URL or a local server
// instead, which doesn't fit this card shape - those stay in Settings.
...
const CLOUD_PROVIDERS = PROVIDERS.filter((p) => CLOUD_PROVIDER_IDS.has(p.id));
...
const EMPTY_PROVIDER_KEY_STATE: ProviderKeyState = {
  value: "",
  saved: false,
  visible: false,
};
...
/**
 * The big-arrival moment before the actual wizard: the logo fades in first,
 * then the welcome line, then the button — one clause at a time, like the
 * app is introducing itself rather than dumping the whole screen at once.
 * Nothing here can be skipped or dismissed; "Get started" is the only way
 * through it.
 */
function WelcomeSplash({ onStart }: { onStart: () => void }) {
  ...
  useEffect(() => {
    const revealText = window.setTimeout(() => setStage(1), 550);
    const revealButton = window.setTimeout(() => setStage(2), 1050);
    return () => {
      window.clearTimeout(revealText);
      window.clearTimeout(revealButton);
    };
  }, []);

  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-7 px-10 text-center select-none">
      <div className="animate-in fade-in zoom-in-90 duration-700 ease-out">
        <VokttyAnimatedLogo size={200} />
      </div>
      {stage >= 1 ? (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 ease-out space-y-2">
      ...
      {stage >= 2 ? (
        <Button
          size="lg"
          onClick={onStart}
          className="animate-in fade-in zoom-in-95 duration-400 ease-out gap-2 rounded-xl px-6 text-sm font-semibold shadow-lg cursor-pointer"
        >
...
export function OnboardingWizard({
  ...
  // Every opening starts over at the welcome splash, not wherever the wizard
  // was left last time.
  useEffect(() => {
    if (!open) return;
    setPhase("welcome");
  ...
  // Load any keys already saved for the cloud providers shown in step 3.
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    void Promise.all(
      CLOUD_PROVIDERS.map(async (p) => {
        const value = await getKey(p.id);
      ...
      setProviderKeys((prev) => {
        ...
        for (const [id, value] of entries) {
          if (!value) continue;
          next[id] = { value, saved: true, visible: false };
        }
        return next;
      });
    });
  ...
  const updateProviderKeyValue = (id: ProviderId, value: string) => {
    setProviderKeys((prev) => ({
      ...prev,
      [id]: { ...providerKeyState(id), value, saved: false },
    }));
  };

  const toggleProviderKeyVisibility = (id: ProviderId) => {
    setProviderKeys((prev) => ({
      ...prev,
      [id]: { ...providerKeyState(id), visible: !providerKeyState(id).visible },
    }));
  };

  const handleSaveProviderKey = async (id: ProviderId) => {
    const trimmed = providerKeyState(id).value.trim();
    if (!trimmed) return;
    try {
      await setKey(id, trimmed);
      ...
      setProviderKeys((prev) => ({
        ...prev,
        [id]: { ...providerKeyState(id), saved: true },
      }));
    } catch (e) {
      console.error(`[voktty] failed to save ${id} key:`, e);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {phase === "welcome" ? (
          <WelcomeSplash onStart={() => setPhase("wizard")} />
        ) : (
          <div className="flex h-full w-full min-w-0 flex-row animate-in fade-in duration-300">
            {/* LEFT SIDEBAR / STEP RAIL */}
            <div className="w-64 shrink-0 bg-muted/15 border-r border-border/40 flex flex-col justify-between p-5">
              <div className="space-y-5">
                {/* Logo & Branding */}
                <div className="flex items-center gap-2.5">
                  <div className="flex size-9 items-center justify-center rounded-lg bg-primary/10 border border-primary/20 shadow-xs">
                    <img src="/voktty.svg" alt="Voktty" className="size-5" />
                  </div>
                  <div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[9px] text-muted-foreground truncate opacity-70">
                            {t(
                              `onboarding.stepRail${s.num}` as "onboarding.stepRail1",
                            )}
                          </div>
                        </div>
                      </button>
            ...
            <div className="flex-1 flex flex-col min-w-0 h-full bg-background/40">
              {/* Header */}
              <div className="flex items-center justify-between px-8 py-3.5 border-b border-border/30 bg-muted/10">
                <div>
              ...
              <div className="flex-1 overflow-y-auto px-8 py-6">
                <div className="mx-auto w-full max-w-2xl space-y-3.5">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5">
                          {SUPPORTED_LANGUAGES.map((lang) => {
                            const selected =
                              (language ?? prefs.language) === lang.id;
                            return (
                              <button
                                key={lang.id}
                                type="button"
                  ...
                  {/* STEP 3: AI providers - one uniform grid, no featured provider */}
                  {step === 3 && (
                    <div className="space-y-3.5">
                      <div>
                        <p className="text-[11px] text-muted-foreground">
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        {CLOUD_PROVIDERS.map((provider) => {
                          const keyState = providerKeyState(provider.id);
                          return (
                            <div
                              key={provider.id}
                              className="rounded-xl border border-border/60 bg-secondary/25 p-2.5 space-y-1.5 shadow-xs"
                            >
                              <div className="flex items-center justify-between gap-1.5">
                                ...
                                {keyState.saved && (
                                  <span className="flex shrink-0 items-center gap-1 text-[9.5px] text-emerald-500 font-medium bg-emerald-500/10 px-1.5 py-0.2 rounded-full">
                                    <HugeiconsIcon
                              ...
                              <div className="relative">
                                ...
                                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      toggleProviderKeyVisibility(provider.id)
                                    }
                                    className="text-muted-foreground hover:text-foreground p-0.5 cursor-pointer"
                                    title={
                                      keyState.visible
                                        ? t("onboarding.hideKey")
                                        : t("onboarding.showKey")
                                    }
                                  >
                                    <HugeiconsIcon
                                      icon={
                                  ...
                                  <Button
                                    ...
                                    disabled={
                                      !keyState.value.trim() || keyState.saved
                                    }
                              ...
                              <button
                                ...
                                className="text-primary hover:underline flex items-center gap-1 cursor-pointer text-[10px]"
                              >
                                {t("onboarding.getProviderKey", {
                        ...
                        <div className="p-2.5 rounded-lg bg-secondary/35 border border-border/60">
                          ...
                          <p className="font-medium text-foreground mt-0.5">
                            {SUPPORTED_LANGUAGES.find(
                              (l) => l.id === (language ?? prefs.language),
              ...
              {/* Footer Navigation */}
              <div className="flex items-center justify-between border-t border-border/30 px-8 py-3 bg-muted/15">
                <Button
                  variant="outline"
                  size="sm"
