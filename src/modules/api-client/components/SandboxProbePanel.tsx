    ...
    <div className="grid h-full grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/50">
      ...
      <div className="flex h-full flex-col overflow-y-auto p-3 bg-background/50">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-foreground">Sandbox Webhook & Idempotency Probe</span>
          </div>
          <Badge variant="outline" className="text-[10px] text-emerald-600 dark:text-emerald-400">
            Zero-Quota Simulator
          </Badge>
        </div>

        {/* Preset Selector */}
        <div className="mb-3 flex flex-col gap-1">
          <span className="text-[11px] font-medium text-muted-foreground">Select Provider Preset:</span>
        ...
        <div className="mb-2.5 flex flex-col gap-1">
          <span className="text-[11px] font-medium text-muted-foreground">Your Local Webhook URL:</span>
          <Input
        ...
        <div className="mb-2.5 flex flex-col gap-1">
          <span className="text-[11px] font-medium text-muted-foreground">
            Signing Secret (Auto generates Stripe-Signature / X-Hub-Signature-256):
          </span>
          <Input
            type="password"
        ...
        <div className="mb-3 flex flex-col gap-1">
          <span className="text-[11px] font-medium text-muted-foreground">Probe Delivery Mode:</span>
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              <span className="text-xs font-semibold">Single Delivery (1x)</span>
              <span className="text-[10px]">Standard webhook event dispatch</span>
            </button>
            ...
            >
              <span className="text-xs font-semibold">Idempotency Probe (3x)</span>
              <span className="text-[10px]">Tests duplicate webhook deduplication</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-1">
          <span className="text-[11px] font-medium text-muted-foreground">Payload (JSON):</span>
          <textarea
            value={payloadText}
        ...
        <div className="mt-3 flex justify-end">
          ...
          >
            ...
            <span>{isDispatchingWebhook ? "Dispatching..." : "Dispatch Webhook Probe"}</span>
          </Button>
        </div>
      </div>
      ...
      <div className="flex h-full flex-col bg-background/50 p-3 overflow-y-auto">
        <div className="mb-2 flex items-center justify-between border-b border-border/50 pb-2">
          <span className="text-xs font-semibold text-foreground">Probe Results & Diagnostics</span>
          {webhookResult && (
            <Button size="sm" variant="ghost" onClick={handleCopyReceipt} className="h-6 gap-1 text-[10.5px]">
              <HugeiconsIcon icon={Copy01Icon} size={11} /> Copy Validation Receipt (.md)
            </Button>
          )}
        </div>
        ...
        {!webhookResult ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center text-muted-foreground/60">
            ...
            <span className="text-xs font-medium">Ready to dispatch sandbox probe</span>
            <span className="max-w-xs text-[11px]">
              Click "Dispatch Webhook Probe" to test your local endpoint against real signed payloads and burst duplicates.
            </span>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold">
                  {webhookResult.isIdempotent ? "Idempotency Probe PASSED" : "Probe FAILED / ERROR"}
                </span>
                <Badge variant={webhookResult.isIdempotent ? "default" : "destructive"} className="text-[10px]">
                  {webhookResult.isIdempotent ? "Verified" : "Fix Needed"}
                </Badge>
              </div>
            ...
            {!webhookResult.isIdempotent && aiAvailable && (
              ...
              >
                ...
                <span>Auto-Fix Webhook Handler with AI Agent</span>
              </Button>
            )}

            {/* Delivery Attempts List */}
            <div className="flex flex-col gap-2">
              <span className="text-[11px] font-semibold text-muted-foreground">Delivery Attempts:</span>
              {webhookResult.attempts.map((att) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-foreground">Attempt #{att.attempt}</span>
