...
import type { VokttyLspClient } from "./client";
...
export type LspPresentationOptions = {
...
export async function acquireDocExtension(
  ...
  const extension: Extension = [
    mod.lspInteractions({
      ...
      onWorkspaceEdit,
      semanticTokens: presentation.semanticTokens,
      inlayHints: presentation.inlayHints,
      onNavigate: (extUri, line, column) => {
        const target = fileUriToPath(extUri);
        const navigator = getLspNavigator();
        if (!target || !navigator) return false;
        navigator.openFile(target, line, column);
        return true;
      },
    }),
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { VokttyLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (VokttyLspClient.hostPid === null) {
    VokttyLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  try {
    ...
    toast.error(t("feedback.lspStartFailed", { name: preset.name }), {
  ...
  const client = new VokttyLspClient({
    transport,
    rootUri,
  ...
  client.setDiagnosticsContext({ owner: key, root });
...
function handleServerExit(key: string): void {
  ...
  recordCrash(key);
  sessions.delete(key);
  managed.client.clearPublishedDiagnostics();
  ...
  if (info?.reason) {
    ...
    toast.error(t("feedback.lspStopped", { name: managed.preset.name }), {
  ...
  if (crashedOut(key)) {
    useLspRuntimeStore
      ...
      .setFailed(
        managed.preset.id,
        tail ? tail.slice(-300) : t("feedback.lspCrashDescription"),
      );
    toast.error(t("feedback.lspCrashing", { name: managed.preset.name }), {
      description: tail
        ? tail.slice(-300)
        : t("feedback.lspGivingUpDescription"),
    });
    return;
  }
  if (tail) {
    toast.error(t("feedback.lspExited", { name: managed.preset.name }), {
...
export async function lspOpenCodeActions(
  view: EditorView,
  onAiFallback: () => void,
): Promise<"opened" | "empty" | "unsupported" | "error"> {
  if (sessions.size === 0) return "unsupported";
  const { openNativeCodeActions } = await import("./client");
  return openNativeCodeActions(view, onAiFallback);
}

export async function lspNavigate(
  view: EditorView,
  kind: import("./client").LspNavigationKind,
): Promise<import("./client").LspNavigationResult> {
  if (sessions.size === 0) return "unsupported";
  const { lspNavigateForView } = await import("./client");
  return lspNavigateForView(view, kind);
}

export async function lspPeek(
  view: EditorView,
  kind: import("./client").LspPeekKind,
): Promise<import("./client").LspPeekResult> {
  if (sessions.size === 0) return { status: "unsupported" };
  const { lspPeekForView } = await import("./client");
  return lspPeekForView(view, kind);
}

export async function lspOpenPeekItem(
  view: EditorView,
  item: import("./client").LspPeekItem,
): Promise<boolean> {
  if (sessions.size === 0) return false;
  const { lspOpenPeekItemForView } = await import("./client");
  return lspOpenPeekItemForView(view, item);
}

export async function lspRequestSignatureHelp(
  view: EditorView,
): Promise<boolean> {
  if (sessions.size === 0) return false;
  const { requestSignatureHelpForView } = await import("./client");
