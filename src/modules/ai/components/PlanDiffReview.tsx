...
import { createLineDiff, formatLineDiffHunkHeader } from "../lib/lineDiff";
...
export function PlanDiffReview() {
  ...
  const lastOperation = usePlanStore((s) => s.lastOperation);
  const lastError = usePlanStore((s) => s.lastError);
  const revertLast = usePlanStore((s) => s.revertLast);
  const dismissLast = usePlanStore((s) => s.dismissLast);
  const [busy, setBusy] = useState(false);

  if (queue.length === 0) {
    if (!lastOperation) return null;
    return (
      <div className="absolute inset-x-2 bottom-2 z-10 rounded-lg border border-border/60 bg-popover/95 p-2 shadow-lg backdrop-blur-xl">
        <div className="flex items-center gap-2">
          <div className="min-w-0 flex-1">
            <div className="text-[11.5px] font-medium text-foreground">
              {lastOperation.status === "applied"
                ? t("ai.planReview.operationApplied")
                : t("ai.planReview.operationReverted")}
            </div>
            <div className="truncate text-[10px] text-muted-foreground">
              {t("ai.planReview.provenance", {
                model: lastOperation.modelId,
                files: lastOperation.files.length,
                commands: lastOperation.commands.length,
              })}
            </div>
            {lastError ? (
          ...
          {lastOperation.status === "applied" ? (
            <Button
              ...
              size="sm"
              variant="outline"
              className="h-7 gap-1 text-[10.5px]"
              disabled={busy}
              onClick={async () => {
                setBusy(true);
                try {
  ...
  const onApply = async () => {
    ...
    try {
      ...
      if (!result.ok) console.error("plan apply failure:", result.error);
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="absolute inset-0 z-10 flex flex-col bg-background/85 backdrop-blur-xl">
      <div className="flex items-center justify-between border-b border-border/40 px-3 py-2">
        <div className="flex flex-col">
          ...
          {lastError ? (
            <span className="text-[10px] text-destructive">{lastError}</span>
          ) : null}
        </div>
        <div className="flex items-center gap-1.5">
          ...
          >
            ...
            {t("ai.diff.acceptAll")} ({queue.length})
          </Button>
        </div>
      </div>
...
function PlanRow({
  ...
  const stats = isDir
    ? null
    : createLineDiff(item.originalContent, item.proposedContent).stats;
    ...
    <li className="group/row overflow-hidden rounded-md border border-border/50 bg-card">
      <div className="flex items-start gap-2 px-2.5 py-1.5">
        <button
          ...
          aria-label={t("tooltips.toggleDiff")}
        >
...
function UnifiedDiffPreview({
  ...
  const { t } = useTranslation();
  const diff = createLineDiff(original, proposed);
  const lines = diff.hunks.flatMap((hunk) => [
    {
      kind: "header" as const,
      text: formatLineDiffHunkHeader(hunk),
    },
    ...hunk.lines,
  ]);
  ...
  return (
    <div className="overflow-hidden rounded border border-border/40 font-mono text-[11px] leading-relaxed">
      <div className="max-h-72 overflow-auto">
        {shown.map((l) => (
          <div
            key={`${l.kind}:${"oldLine" in l ? `${l.oldLine}:${l.newLine}` : l.text}`}
            className={cn(
              "flex whitespace-pre",
              l.kind === "add"
