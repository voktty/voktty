export { KEYRING_SERVICE } from "@/lib/identity";
...
export type CompatibleCompletionProfile = Exclude<
...
export const MODELS = [
  ...
  },
  {
    ...
    hint: "Flagship",
    description: "Deep reasoning, 1M context, strong coding capabilities.",
    capabilities: { intelligence: 5, speed: 3, cost: 2 },
    tags: ["vision", "reasoning", "tools", "coding"],
  },
  {
...
export const MODEL_PRICING: Record<string, ModelPricing> = {
  ...
  "claude-fable-5-1": { input: 8, output: 40, cacheRead: 0.25 },
  ...
  "gemini-3.7-flash": { input: 0.3, output: 2.5, cacheRead: 0.075 },
  "gemini-3.6-flash": { input: 0.2, output: 1.5, cacheRead: 0.05 },
...
export type SttProvider = "openai" | "groq" | "whispercpp" | "browser";
...
export const DEFAULT_STT_PROVIDER: SttProvider = "browser";
...
export const SYSTEM_PROMPT = `You are Voktty, an AI agent embedded in a developer terminal emulator. You are a hands-on engineer, not a chat bot — your job is to *do* the work, not narrate it.

# Environment
Every turn carries a short <env> block (prepended to the latest user message): workspace_root, active_terminal_cwd, optionally active_file. Treat it as ground truth — never ask the user where they are. When active_file is present in <env>, it is the file currently open in the editor tab directly in front of the user. Unless the user specifies a different target or explicitly refers to workspace directory browsing, assume questions about "this", "here", code generation, fixes, modifications, or explanations pertain to active_file (call read_file on active_file first). The terminal scrollback is NOT auto-injected; call get_terminal_output only when the user references "this error" / "the last command" or you genuinely need to interpret recent output.
...
- "edit/fix this file", "write code", "explain this" with no path → active_file when present (call read_file first).
...
export const SYSTEM_PROMPT_LITE = `You are Voktty, an AI agent in a developer terminal. Each turn carries an <env> block (workspace_root, active_terminal_cwd, optional active_file) prepended to the user's message — treat as ground truth. When active_file is present, it is the open file in the editor: target it for unaddressed code generation, edits, fixes, or explanations (call read_file on active_file first).
