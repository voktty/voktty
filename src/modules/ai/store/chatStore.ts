...
import { isAiRuntimeAvailable } from "../lib/runtimeAvailability";
import {
...
export type FileCitationSource =
  | { kind: "local"; root: string | null }
  | { kind: "collab"; leafId: number; available: boolean };
...
export type Live = {
  ...
  getFileCitationSource: () => FileCitationSource;
  getActiveFile: () => string | null;
  getEditorBuffers: () => Promise<DevelopmentBuffer[]>;
  getWorkspaceDiagnostics: () => DevelopmentDiagnostic[];
...
const NOOP_LIVE: Live = {
  ...
  getFileCitationSource: () => ({ kind: "local", root: null }),
  getActiveFile: () => null,
...
const CHAT_PANEL_OPEN_STORAGE_KEY = "voktty.chat.panelOpen";
const LAST_MODEL_STORAGE_KEY = "voktty.ai.lastModel";

function readInitialModelId(): ModelId {
  try {
...
function persistSelectedModelId(id: string) {
  try {
    window.localStorage.setItem(LAST_MODEL_STORAGE_KEY, id);
  } catch {
    // ignore
  }
}
...
function readInitialPanelOpen(): boolean {
  try {
    return window.localStorage.getItem(CHAT_PANEL_OPEN_STORAGE_KEY) === "1";
  } catch {
    return false;
  }
}
...
function persistPanelOpen(open: boolean) {
  try {
    window.localStorage.setItem(CHAT_PANEL_OPEN_STORAGE_KEY, open ? "1" : "0");
  } catch {
    // ignore
  }
}
...
export const useChatStore = create<StoreState>((set, get) => ({
  ...
  hydrateSessions: async () => {
    ...
    if (sessions.length > 0) {
      ...
      targetId = validActive ? activeId! : sessions[0].id;
    } else {
      targetId = newSessionId();
      const fresh: SessionMeta = {
        id: targetId,
        title: translate("ai.newChat"),
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
      nextSessions = [fresh];
    ...
    try {
      ...
      if (msgs && msgs.length > 0 && !chats.has(targetId)) {
        seedMessages.set(targetId, msgs);
      }
    } catch {
      // ignore
    }
