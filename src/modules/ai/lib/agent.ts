...
import { loadMcpToolsForRun } from "../tools/mcp";
...
const TOOL_LABELS: Record<string, (input: Record<string, unknown>) => string> =
  {
    ...
    development_context: () => `Collecting development context`,
    run_development_check: (i) =>
      `Running ${ellipsize(String(i.command ?? "development check"), 60)}`,
...
export function buildConfiguredLanguageModel(
  ...
  // A per-model local agent id carries its own model; the adapter reads it
  // from the id, so nothing else here has to know about the catalog.
  if (isHarnessModelId(modelId)) {
    return buildLanguageModel("harness", keys, modelId, {});
  }
...
Call development_context before planning non-trivial changes. Mutating tools (write_file, edit, multi_edit, create_directory) will queue their changes for the user to review as one reversible multi-file operation. Do NOT execute bash_run, bash_background, or run_development_check while plan mode is active — restrict yourself to reads and queued mutations. After queueing the full set of edits, stop and return a brief summary; do not continue acting until the user has accepted/rejected.`;
...
export function buildStableSystem(
  ...
  const langObj = SUPPORTED_LANGUAGES.find((l) => l.id === languageId);
  const langLabel = langObj
    ? `${langObj.label} (${langObj.id})`
    : "English (en)";
  const langBlock = `\n\n## LANGUAGE PREFERENCE & LOCALIZATION
- Interface display language: ${langLabel}.
- You MUST communicate, explain, answer questions, and formulate your natural language responses in ${langLabel}.
- Code syntax, terminal commands, file paths, and technical identifiers remain in standard syntax, but all explanations, summaries, comments, and conversation MUST be in ${langLabel}.`;
  ...
  const memoryBlock =
    projectMemory && projectMemory.trim().length > 0
      ? `\n\n## PROJECT — VOKTTY.md\n${projectMemory.trim()}`
      : "";
  return `${base}${langBlock}${memoryBlock}${personaBlock}${customBlock}`;
}
...
export async function runAgentStream(opts: RunAgentOptions) {
  ...
  const mcpTools = await loadMcpToolsForRun(opts.uiMessages);
  ...
  return streamText({
    ...
    tools: buildTools(opts.toolContext, mcpTools),
