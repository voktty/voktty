...
function workspaceComparisonForm(path: string): string | null {
  let normalized = path.replace(/\\/g, "/");
  const drive = normalized.match(/^([a-zA-Z]):\//);
  const isAbsolute = normalized.startsWith("/") || drive !== null;
  ...
  for (const segment of normalized.split("/")) {
    ...
    segments.push(drive ? segment.toLowerCase() : segment);
  }
  return `${prefix}/${segments.join("/")}`.replace(/\/$/, "") || "/";
}

/**
 * Restricts an already-resolved path to the active workspace. Canonical-path
 * checks must still run before this function so symlink targets are compared,
 * not only their user-facing aliases.
 */
export function checkWorkspacePath(
  ...
  if (!workspaceRoot) {
    return { ok: false, reason: "Refused: no active workspace root." };
  }
  const candidate = workspaceComparisonForm(path);
  const root = workspaceComparisonForm(workspaceRoot);
  if (!candidate || !root) {
    return { ok: false, reason: "Refused: workspace paths must be absolute." };
  }
  const inside =
    root === "/"
  ...
  return inside
    ? { ok: true }
    : { ok: false, reason: "Refused: path is outside the active workspace." };
}

export async function checkCanonicalWorkspacePath(
  canonicalPath: string,
  workspaceRoot: string | null,
  canonicalize: (path: string) => Promise<string>,
): Promise<SafetyResult> {
  if (!workspaceRoot) return checkWorkspacePath(canonicalPath, null);
  try {
    const canonicalRoot = await canonicalize(workspaceRoot);
    return checkWorkspacePath(canonicalPath, canonicalRoot);
  } catch {
    return {
      ok: false,
      reason: "Refused: the active workspace root could not be verified.",
    };
  }
}
