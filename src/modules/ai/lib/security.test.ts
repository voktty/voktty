...
describe("checkWorkspacePath — workspace boundary", () => {
  it("allows the root and descendants on POSIX", () => {
    expect(checkWorkspacePath("/repo", "/repo")).toMatchObject({ ok: true });
    expect(checkWorkspacePath("/repo/src/app.ts", "/repo")).toMatchObject({
      ok: true,
    });
  });

  it("rejects prefix look-alikes and parent traversal", () => {
    expect(checkWorkspacePath("/repo-other/app.ts", "/repo")).toMatchObject({
      ok: false,
    });
    expect(checkWorkspacePath("/repo/../outside.txt", "/repo")).toMatchObject({
      ok: false,
    });
  });

  it("normalizes separators and drive casing on Windows", () => {
    expect(
      checkWorkspacePath("c:\\Work\\Repo\\src\\app.ts", "C:/work/repo"),
    ).toMatchObject({ ok: true });
    expect(
      checkWorkspacePath("C:\\Work\\Repository\\app.ts", "C:/work/repo"),
