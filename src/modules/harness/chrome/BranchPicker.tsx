...
export function BranchPicker({
  ...
  const createName = useMemo(() => {
    ...
    return taken ? "" : name;
  }, [projectBranches, query]);

  const createRow = useMemo((): CreateRow | null => {
    if (query.trim()) {
      return createName ? { kind: "create", name: createName } : null;
    }
      ...
      <div ref={root} className="relative min-w-0">
        ...
        {open ? (
          ...
          >
            ...
            {createRow ? (
              <div className="shrink-0 border-t border-content/10 p-1 px-1.5">
                <button
                  type="button"
                  disabled={busy}
                  onMouseDown={(e) => e.preventDefault()}
                ...
                >
                  ...
                  <span className="min-w-0 truncate">
                    {createRow.name
                      ? t("harness.chrome.createAndCheckout", {
                          name: createRow.name,
                        })
                      : t("harness.chrome.newBranch")}
                  </span>
                </button>
              </div>
            ) : null}
