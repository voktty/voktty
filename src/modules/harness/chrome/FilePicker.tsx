...
type RankedAction = Action & {
  score: number;
  positions: number[];
};

function reloadActionHint(): string {
  return `${MOD}${SHIFT}R`;
}

const ACTIONS: Action[] = [
  { id: "reload", label: "Reload Voktty", hint: reloadActionHint() },
];
...
export function FilePicker({
  ...
  const content = (
    <div className="fixed inset-0" style={{ zIndex: LAYER.dialog }}>
      ...
      <div
        role="dialog"
        aria-label={paletteMode ? "Command Palette" : t("harness.chrome.goToFile")}
        data-file-picker
          ...
          <label className="flex items-center gap-2 border-b border-content/10 px-2 py-2.5 text-content/50">
            ...
            <input
              ...
              placeholder={
                paletteMode
                  ? "Command Palette"
                  : `${t("harness.chrome.goToFile")} (type > for commands)`
              }
              aria-label={paletteMode ? "Command Palette" : t("harness.chrome.goToFile")}
              spellCheck={false}
              autoComplete="off"
