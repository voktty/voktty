...
export function ToolDiffPreview({
  ...
  const description =
    status === "pending"
      ? t("toolPreview.proposedChanges")
      : status === "rejected"
        ? t("toolPreview.attemptedChanges")
        : preview.contentOnly
          ? t("toolPreview.writtenContent")
          : t("toolPreview.changePreview");
  ...
  return (
    <>
      <button
        ...
        aria-label={
          onOpen
            ? t("toolPreview.openLabel", { label })
            : t("toolPreview.previewLabel", {
                type: preview.contentOnly ? "written content" : "changes",
                label,
              })
        }
      ...
      {open ? (
        ...
        >
          <div className="flex items-center gap-2 border-b border-content/8 px-2.5 py-1.5 font-sans text-[11px] text-content/50">
            ...
            <button
              type="button"
              aria-label={t("toolPreview.closePreview")}
              className="shrink-0 rounded p-0.5 hover:bg-content/8 hover:text-content cursor-pointer"
