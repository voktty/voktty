...
class MockClipboardItem {
  ...
  async getType(type: string) {
    if (!this.data[type]) throw new Error(`Type ${type} not found`);
...
if (!globalThis.navigator?.clipboard) {
  ...
  const originalReadText = globalThis.navigator.clipboard.readText?.bind(globalThis.navigator.clipboard);
  if (!originalWriteText) {
    globalThis.navigator.clipboard.writeText = async (text: string) => {
      mockClipboardText = text;
    };
  }
  if (!originalReadText) {
    globalThis.navigator.clipboard.readText = async () => mockClipboardText;
  }
}
