...
describe("unwrapShellCommand", () => {
  it("unwraps POSIX shells without including trailing shell arguments", () => {
    expect(
      unwrapShellCommand(`/bin/zsh -lc "npm test -- --run app.test.ts"`),
    ).toBe("npm test -- --run app.test.ts");
    expect(
      unwrapShellCommand(`/bin/zsh -lc "rg -n \\"foo\\" src" ignored`),
    ).toBe('rg -n "foo" src');
  });
  ...
  it("stops scanning PowerShell launcher options at -File", () => {
    expect(
      unwrapShellCommand(`pwsh -File script.ps1 -Mode -Command build`),
    ).toBe(`pwsh -File script.ps1 -Mode -Command build`);
