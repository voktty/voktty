...
const LOGIN_CHILD_PREFIX = "voktty-provider-login-";
 ...
 * credential storage; Voktty only supervises its exit status. Duplicate
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const account =
      accountId && accountId !== "default" && supportsProviderAccounts(harness)
        ...
        : undefined;
    const spawn = account
      ? child.spawnChild(childId, path, [...args], cwd, null, account)
