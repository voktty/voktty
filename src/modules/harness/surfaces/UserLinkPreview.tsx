...
import {
  ...
  type UserLink,
} from "@/modules/harness/lib/linkPreview";
...
export function UserLinkPreview({ link }: { link: UserLink }) {
  ...
  return (
    <a
      ...
      title={link.url}
      aria-label={title}
      className="user-link-preview group mx-0.5 inline-flex items-center text-sky-400/90 hover:text-sky-300 hover:underline"
      onClick={(event) => {
    ...
    >
      <span className="mr-1 inline-flex size-4 items-center justify-center overflow-hidden rounded bg-foreground/10 align-[-0.125em] text-[9px] font-semibold uppercase text-foreground/55">
        {favicon ? (
          <img
            src={favicon}
            alt=""
