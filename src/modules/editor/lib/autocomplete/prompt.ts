...
export function buildUserPrompt(req: CompletionRequest): string {
  ...
  if (req.context?.currentBlock) {
    contextParts.push(
      `CURRENT LOGICAL BLOCK:\n<<<\n${req.context.currentBlock.slice(0, 6000)}\n>>>`,
    );
  }
  if (req.context?.neighborBefore) {
    contextParts.push(
      `NEIGHBORING CODE BEFORE:\n<<<\n${req.context.neighborBefore.slice(-2000)}\n>>>`,
    );
  }
  if (req.context?.neighborAfter) {
    contextParts.push(
      `NEIGHBORING CODE AFTER:\n<<<\n${req.context.neighborAfter.slice(0, 2000)}\n>>>`,
    );
  }
  if (req.context?.symbols.length) {
    contextParts.push(`SYMBOLS:\n${req.context.symbols.slice(0, 12).join("\n")}`);
  }
  if (req.context?.diagnostics.length) {
    contextParts.push(
      `DIAGNOSTICS:\n${req.context.diagnostics.slice(0, 6).join("\n")}`,
    );
  }
