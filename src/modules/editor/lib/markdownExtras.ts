...
export function markdownCodeLanguages(): LanguageDescription[] {
  ...
  mdCodeLanguages = LANGUAGES.filter(
    (l) => l.name !== "Markdown" && l.name !== "Voktty Theme",
  ).map((l) =>
...
function createUrlHighlighter(): Extension {
  const urlMatcher = new MatchDecorator({
    regexp: URL_RE,
    decoration: Decoration.mark({
      class: "cm-md-url",
      attributes: { title: translate("editor.markdownOpenLink") },
    }),
  });
...
export function markdownExtras(): Extension {
  return [createUrlHighlighter(), clickHandlers, mdTheme];
}
