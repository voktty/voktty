import { LEGACY_STORAGE_PATHS } from "@/lib/identity";

const DB_NAME = "voktty-bg-images";
const LEGACY_DB_NAME = LEGACY_STORAGE_PATHS.bgImages;
const STORE = "images";
const VERSION = 1;
...
function readLegacyBgImage(id: string): Promise<Blob | null> {
  return new Promise((resolve) => {
    const req = indexedDB.open(LEGACY_DB_NAME, VERSION);
    req.onupgradeneeded = () => {
...
export async function getBgImage(id: string): Promise<Blob | null> {
  const db = await openDb();
  const current = await new Promise<Blob | null>((resolve, reject) => {
    const tx = db.transaction(STORE, "readonly");
  ...
  const legacy = await readLegacyBgImage(id);
